...
import {
  engine,
  world,        // まだ main で使うなら残す（savePanel など）
  render,       // resize を移したら不要になるかも
  canvas,       // クリック解除で使うなら残す
  taskBodies,
  BLOCK_W,      // updateLabels が使うなら残す
  BLOCK_H,      // resize 移したら不要かも
  getSize,      // resize 移したら不要
  createBounds, // resize 移したら不要
  addBlock,
  removeBlock,
} from './js/physics.js';
...
const { Bodies, Body, World, Events, MouseConstraint } = Matter;
...
canvas.addEventListener('click', () => { state.selectedTaskId = null; });
...
document.addEventListener('keydown', (e) => {
  ...
  if (e.key === 'Delete') {
    ...
    e.preventDefault();
    toggleDone(state.selectedTaskId);
...
function updateStats() {
  document.getElementById('s-total').textContent = state.tasks.length;
